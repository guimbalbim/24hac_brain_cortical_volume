...
#ggsave(plot = tern_plot, "../04.figures/tern_plot_20241002.pdf", dpi = 300, height = 13, width = 18)
```
...
# FDR
sleep_lhrh_vol_gls_results_fdr <- sleep_lhrh_vol_gls_results %>% 
  mutate(fdr = p.adjust(pvalue, method = "BH")) %>% 
  filter(fdr <= 0.05)
sleep_lhrh_vol_gls_results_fdr

```

###### With addition of average day duration
```{r, message=FALSE}
overallcomp_lhrh_vol_gls_lm = list()
...
for (var in regions_vol_gls){
# Formula
form <- paste(var, "~ cbind(ilr_mvpa1,ilr_mvpa2,ilr_mvpa3) + age + education + sex + rct + eicv + day_duration", sep = "")
...
for (var in regions_vol_gls){
# Formula
form <- paste(var, "~ ilr_mvpa1 + age + education + sex + rct + eicv + day_duration", sep = "")
...
for (var in regions_vol_gls){
# Formula
form <- paste(var, "~ ilr_lpa1 + age + education + sex + rct + eicv + day_duration", sep = "")
...
for (var in regions_vol_gls){
# Formula
form <- paste(var, "~ ilr_sb1 + age + education + sex + rct + eicv + day_duration", sep = "")
...
for (var in regions_vol_gls){
# Formula
form <- paste(var, "~ ilr_sleep1 + age + education + sex + rct + eicv + day_duration", sep = "")
...
#ggsave("../04.figures/combined_unc_plots_leg_20241002.pdf", plot = combined_unc_plots_leg, width = 16, height = 14, dpi = 300)
...
#ggsave("../04.figures/combined_fdr_plots_20241003.pdf", plot = combined_fdr_plots, width = 12, height = 16, dpi = 600)
...
## Get ilrs as per transformation from predict_delta_comps
its_data <- its_data %>% 
  mutate(ilr1 = sqrt(3/4) * log(Sleep / (Sedentary_behavior * Light_PA * MVPA)^(1/3)),
         ilr2 = sqrt(2/3) * log(Sedentary_behavior / (Light_PA * MVPA)^(1/2)),
         ilr3 = sqrt(1/2) * log(Light_PA / MVPA))

its_te2a_model <- lm(
  rh_R_TE2a_ROI_volume_cm_gls ~ ilr1 + ilr2 + ilr3 + age + education + sex + eicv + rct, 
  data = its_data
)
summary(its_te2a_model)
...
its_plot <- plot_delta_comp(
  ...
  labs(x = paste0("Change in 24-HAC movement behavior composition (min/day)"),
       y = paste0("Predicted change in right inferior temporal gyrus (TE2a region) volume with 95% CI"))
its_plot

## New Plot
### Function (author: Narlon C. Boa Sorte Silva)
predict_delta_comps_rev <- function(mydata, comp_total, units_lab){

  # Outcome of interest
  outc <- attr(mydata, "y")
  # Levels
  comps <- attr(mydata, "comps")
  comparisons <- attr(mydata, "comparisons")

  # Significant level and CI
  alph <- attr(mydata, "alpha")
  ci_lev <- sprintf("%2.0f", 100 * (1 - alph))

  # Create factors
  mydata$`comp+` <- factor(mydata$`comp+`, levels = comps)

  # Make sure levels are ordered by the order they are specified in the function call
  mydata$`comp+` <- factor(mydata$`comp+`, levels = comps)
  levels(mydata$`comp+`) <- paste0(comps, "+Delta")
...
# Generate plot
its_plot_new <- predict_delta_comps_rev(
  mydata = its_te2a,
  comp_total = 1440, 
  units_lab = "min")
...
#ggsave(plot = its_plot_new,  "../04.figures/isotemp_subs_20241002.pdf", dpi = 300, height = 8, width = 8)
...
## Percentage
delta_mvpa_sb <- c(-0.08297087, -0.05175327, -0.02442656,  0.00000000,  0.02218660,  0.04259557,  0.06156358)
avg_vol <- 2.90
...
# Adding SB and removing MVPA
## Filtering
its_te2a_addsb_remmvpa <- its_te2a %>% 
  group_by(comp_plus) %>% 
  filter(comp_minus == "MVPA") %>% 
  ungroup() %>% 
  filter(comp_plus == "Sedentary_behavior")

## Extracting values
its_te2a_addsb_remmvpa$delta_pred
its_te2a_addsb_remmvpa$ci_lo
its_te2a_addsb_remmvpa$ci_up
its_te2a_addsb_remmvpa$sig
...
its_prorata_te2a <- 
    predict_delta_comps(
        ...
        covars = c("age", "education", "sex", "eicv", "rct"),
        deltas =  seq(-30, 30, by = 10) / (24 * 60), 
        comparisons = "prop-realloc", # one-v-one shows replacing one with another and prop-realloc replacing one with any other
        alpha = 0.05
    )

# Estimates
attr(its_prorata_te2a, "mean_pred")
...
# Plotting
## New Plot
### Function (authors: Narlon C. Boa Sorte Silva and Guilherme M. Balbim)
predict_delta_comps_prop_rev <- function(mydata, comp_total, units_lab){
...
# Generate plot
its_prorata_plot_new <- predict_delta_comps_prop_rev(
  mydata = its_prorata_te2a,
  comp_total = 1440, 
  units_lab = "min")

# Show plot
its_prorata_plot_new

#ggsave(plot = its_prorata_plot_new,  "../04.figures/isotemp_subs_prop_20250324.jpg", dpi = 600, height = 8, width = 8)

# Extracting delta predicted and upper/lower 95%CI
## Cleaning special characters
names(its_prorata_te2a) <- gsub(x = names(its_prorata_te2a), pattern = "\\+", replacement = "_plus")
names(its_prorata_te2a) <- gsub(x = names(its_prorata_te2a), pattern = "\\-", replacement = "_minus")

# Adding Sleep and removing proportional LPA, MVPA and SB
## Filtering 
its_te2a_addspl_remothers <- its_prorata_te2a %>% 
  group_by(comp_plus) %>% 
  filter(comp_minus == "proportional-realloc") %>% 
  ungroup() %>% 
  filter(comp_plus == "Sleep")

## Extracting values
its_te2a_addspl_remothers$delta_pred
its_te2a_addspl_remothers$ci_lo
its_te2a_addspl_remothers$ci_up
its_te2a_addspl_remothers$sig

# Adding SB and removing proportional LPA, MVPA and SL
## Filtering 
its_te2a_addsb_remothers <- its_prorata_te2a %>% 
  group_by(comp_plus) %>% 
  filter(comp_minus == "proportional-realloc") %>% 
  ungroup() %>% 
  filter(comp_plus == "Sedentary_behavior")
...
## Extracting values
its_te2a_addsb_remothers$delta_pred
its_te2a_addsb_remothers$ci_lo
its_te2a_addsb_remothers$ci_up
its_te2a_addsb_remothers$sig

# Adding LPA and removing proportional SB, MVPA and SL
## Filtering 
its_te2a_addlpa_remothers <- its_prorata_te2a %>% 
  group_by(comp_plus) %>% 
  filter(comp_minus == "proportional-realloc") %>% 
  ungroup() %>% 
  filter(comp_plus == "Light_PA")
...
## Extracting values
its_te2a_addlpa_remothers$delta_pred
its_te2a_addlpa_remothers$ci_lo
its_te2a_addlpa_remothers$ci_up
its_te2a_addlpa_remothers$sig

# Adding MVPA and removing proportional SB, LPA and SL
## Filtering 
its_te2a_addmvpa_remothers <- its_prorata_te2a %>% 
  group_by(comp_plus) %>% 
  filter(comp_minus == "proportional-realloc") %>% 
  ungroup() %>% 
  filter(comp_plus == "MVPA")
...
## Extracting values
its_te2a_addmvpa_remothers$delta_pred
its_te2a_addmvpa_remothers$ci_lo
its_te2a_addmvpa_remothers$ci_up
its_te2a_addmvpa_remothers$sig

## Percentage
delta_mvpa_others <- c(-0.07557060, -0.04677190, -0.02191141,  0.00000000,  0.01962078,  0.03741146,  0.05370681)
avg_vol <- 2.90
pct_change_mvpa_others <- (delta_mvpa_others / avg_vol)*100
pct_change_mvpa_others

```

## With the addition of day duration
### One-to-one reallocation
```{r, message=FALSE}
its_data <- all_data_gls %>% 
  dplyr::select("id","adas_cog_plus","mvpa_min_clos","mvpa_pct_clos","sedentary_min_clos","sedentary_pct_clos",
                "light_pa_min_clos","light_pa_pct_clos","sleep_min_clos","sleep_pct_clos",
                "age", "education", "sex", "eicv", "rct", "day_duration","rh_R_TE2a_ROI_volume_cm_gls") %>% 
  mutate(rct = as.factor(rct),
         education = as.factor(education)) %>% 
  rename("Sleep" = "sleep_min_clos",
         "Sedentary_behavior" = "sedentary_min_clos",
...
its_te2a <- 
    predict_delta_comps(
        ...
        comps = c("Sleep",
                  ...
                  "Light_PA",
                  "MVPA"),
        covars = c("age", "education", "sex", "eicv", "rct", "day_duration"),
        deltas =  seq(-30, 30, by = 10) / (24 * 60), 
        comparisons = "one-v-one", # one-v-one shows replacing one with another and prop-realloc replacing one with any other
        alpha = 0.05
    )
...
its_te2a_model <- lm(
  rh_R_TE2a_ROI_volume_cm_gls ~ ilr1 + ilr2 + ilr3 + age + education + sex + eicv + rct + day_duration, 
  data = its_data
)
summary(its_te2a_model)
